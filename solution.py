...
try:
    ...
    if challengeLevel == 3:
        ...
        controller = ControlFlow(control, camera, imu, path=path, lidar=lidar)
        controller.search_for_tag(6, -1, 10)
        while rclpy.ok():
            rclpy.spin_once(robot, timeout_sec=0.1)
            time.sleep(0.1)
            controller.make_move(0.1)
    ...
    if challengeLevel == 7:
        # print("TEST 6 OMG")
        controller = ControlFlow(control, camera, imu, mode=ROBOTMODE.STOPSIGN, lidar=lidar)
        # controller.search_for_tag(5, -1, 360)
        while rclpy.ok():
            rclpy.spin_once(robot, timeout_sec=0.1)
            time.sleep(0.1)
